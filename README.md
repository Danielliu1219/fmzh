...
| 登录 | 首页 |
|---|---|
| ![](docs/images/01_login.png) | ![](docs/images/02_home.png) |

| 考试管理 | 复习资料 | AI 复习分析 |
|---|---|---|
| ![](docs/images/03_exams.png) | ![](docs/images/04_materials.png) | ![](docs/images/05_analysis.png) |

界面为实际运行截图（首页为本地预览数据，其余为真实接口数据）。

## 项目状态

- **需求分析已完成并交付**：需求分析文档 V1.2 共 87 页，9 个模块 57 个用例、68 张 UML 图、数据字典、72 条需求对照记录（**课程交付文档不随本仓库发布**，67 条用户需求全文已并入本文档第 2 节）
...
1. **分析对象就是用户上传的资料** —— 历年题、老师划的重点、PPT、课堂笔记、作业、练习题，系统不去网上搜资料。这样更贴近期末复习的真实场景，也避免大模型随意发挥；
2. **用结构化输入管住大模型** —— 「用户输入结构化 + 固定提示词 + 限制输出格式」三件套，让分析结果稳定、可复现；
3. **自然语言先转成固定 JSON**，作为后续分析的依据：
...
```json
{
  ...
  "daily_hours": 3,
  "weak_chapters": ["极限", "积分"],
  "materials": ["历年题", "老师重点", "PPT"],
  "output_need": ["知识点权重", "复习计划", "自测题"]
}
```

4. **大模型输出也是固定格式** —— 知识点列表、权重分析、常考必考判断、推荐依据、个性化计划、自测题，每一项都有约定好的结构。

---

## 2. 功能总览（谁要做什么，看这张表）

系统功能需求共 **9 个模块、57 个用例**，完整定义见课程提交的需求分析文档；67 条用户需求全文见本节末尾。状态含义：
...
### 用户需求清单（FMZH-USR-0001 ~ 0067）

系统共 67 条用户需求，编号连续无缺号。

- **FMZH-USR-0001** 学生可注册新账号，注册时需提交账号、密码、姓名、学号、专业等信息。
- **FMZH-USR-0002** 系统应校验注册账号的唯一性，当账号已存在时提示用户重新输入。
- **FMZH-USR-0003** 学生可通过账号和密码登录系统。
- **FMZH-USR-0004** 学生连续三次输入错误密码后，系统应锁定该账号并提示用户进行找回或等待解锁。
- **FMZH-USR-0005** 学生可编辑个人资料，包括头像、姓名、专业、年级、班级等信息。
- **FMZH-USR-0006** 学生可注销个人账号，系统应在确认后处理相关用户数据。
- **FMZH-USR-0007** 学生登录后，系统应展示“今日待办”聚合视图，包括考试、任务、提醒和复习进度。
- **FMZH-USR-0008** 学生可录入个人考试安排，包括科目、时间、地点、时长等信息。
- **FMZH-USR-0009** 学生可修改或删除已录入的个人考试安排。
- **FMZH-USR-0010** 学生可一键导入由教师或管理员发布的统一考试安排到个人时间表。
- **FMZH-USR-0011** 系统自动生成考试倒计时并在首页展示。
- **FMZH-USR-0012** 学生可按科目或日期筛选查看自己的考试列表。
- **FMZH-USR-0013** 学生可为各科目创建备考计划，并添加学习任务。
- **FMZH-USR-0014** 学习任务应包含名称、科目、起止时间、地点、优先级、难度等信息。
- **FMZH-USR-0015** 学生可编辑或删除已创建的学习任务。
- **FMZH-USR-0016** 学习任务支持设置重复规则，包括每天、每周和自定义。
- **FMZH-USR-0017** 系统可根据考试时间倒推自动生成备考时间表。
- **FMZH-USR-0018** 系统应自动检测同一时段内的多个任务冲突并提示用户。
- **FMZH-USR-0019** 学生可拖拽调整任务时间，并支持日视图、周视图、月视图切换。
- **FMZH-USR-0020** 学生可为单个任务设置提醒时间。
- **FMZH-USR-0021** 系统应提供番茄钟计时功能帮助学生专注学习。
- **FMZH-USR-0022** 学生可设定每日学习时长目标，系统统计达成情况。
- **FMZH-USR-0023** 学生可为各科目设置颜色标签进行区分。
- **FMZH-USR-0024** 系统支持外部日历 ICS 导入和导出。
- **FMZH-USR-0025** 学生可上传复习资料，支持 PDF、Word、PPT、TXT、图片等格式。
- **FMZH-USR-0026** 系统应校验上传资料的文件格式、大小和安全性。
- **FMZH-USR-0027** 学生上传资料时可添加标题、科目、标签和描述信息。
- **FMZH-USR-0028** 学生可编辑、删除或移动分类已上传的复习资料。
- **FMZH-USR-0029** 学生可上传多种类型的复习信息，包括历年试题、老师标注重点、平时作业、课堂笔记、教材章节、实验报告和练习题。
- **FMZH-USR-0030** 系统应将同一科目的多份资料进行统一整理，形成该科目的综合复习资料库。
- **FMZH-USR-0031** 系统可识别不同资料来源，并区分教师重点、历年题、课堂笔记、作业练习等资料类型。
- **FMZH-USR-0032** 系统可对复习资料进行全文搜索，并按科目、标签、日期和资料类型筛选排序。
- **FMZH-USR-0033** 学生可将重点、错题或高频题标记到“错题本/收藏”。
- **FMZH-USR-0034** 学生可将多份资料合并为一份复习笔记，并导出为 PDF 或 Markdown。
- **FMZH-USR-0035** 系统调用大模型自动提取复习资料中的核心知识点、摘要和复习提纲。
- **FMZH-USR-0036** 系统调用大模型生成思维导图、知识框架、记忆卡片和背诵要点。
- **FMZH-USR-0037** 学生可针对资料向大模型提问并获得解答，系统保存对话历史并支持多轮追问。
- **FMZH-USR-0038** 系统调用大模型根据资料生成自测练习题。
- **FMZH-USR-0039** 系统应基于多来源复习资料综合分析常考题型、必考知识点和高频考点。
- **FMZH-USR-0040** 系统应根据历年题出现频率、教师强调次数、作业覆盖情况和资料标注信息，计算各知识点考试权重。
- **FMZH-USR-0041** 系统应以可视化方式展示每个知识点的重要程度、掌握状态和建议投入时间。
- **FMZH-USR-0042** 系统应允许学生选择备考目标，例如“保及格”“稳中等”“冲高分”等。
- **FMZH-USR-0043** 系统应根据学生的备考目标、剩余时间、历史学习记录和自测结果，生成个性化知识点权重。
- **FMZH-USR-0044** 对于基础薄弱或只求及格的学生，系统应优先推荐高频基础题、必考概念和短期提分内容。
- **FMZH-USR-0045** 对于基础较好且追求高分的学生，系统应推荐综合题、难题、易失分点和拓展考点。
- **FMZH-USR-0046** 系统应根据用户完成任务情况和自测表现动态调整复习计划和知识点权重。
- **FMZH-USR-0047** 系统应解释个性化推荐依据，例如该知识点来自历年高频题、教师重点或用户薄弱项。
- **FMZH-USR-0048** 系统应限制大模型输出范围，使其回答尽量基于用户上传资料、课程资料和考试目标。
- **FMZH-USR-0049** 系统应提示大模型生成内容仅供复习参考，用户需结合教材和课堂要求判断。
- **FMZH-USR-0050** 学生可对学习任务打卡标记完成。
- **FMZH-USR-0051** 系统自动统计各科目学习时长，并以图表展示学习进度和任务完成率。
- **FMZH-USR-0052** 系统生成备考周报或学习总结，结合大模型评测给出各科目掌握程度评分。
- **FMZH-USR-0053** 学生可查看历史学习记录统计报表。
- **FMZH-USR-0054** 系统在考试前和任务开始前发送站内或邮件提醒。
- **FMZH-USR-0055** 学生可自定义提醒方式和提醒时间。
- **FMZH-USR-0056** 学生可查看通知历史，并标记已读、未读或删除。
- **FMZH-USR-0057** 系统可生成“期末压力指数”，综合考试密度、剩余任务量和完成率提示学生调整计划。
- **FMZH-USR-0058** 系统可提供“临时救急模式”，在考试前短时间内生成高优先级冲刺复习清单。
- **FMZH-USR-0059** 系统可根据错题本、收藏内容和高权重知识点生成考前重点回顾列表。
- **FMZH-USR-0060** 系统可提供“今日最重要的三件事”功能，帮助学生降低期末周任务焦虑。
- **FMZH-USR-0061** 系统可根据学生学习习惯推荐适合的学习时间段。
- **FMZH-USR-0062** 系统可提供轻量级情绪记录功能，并根据压力状态给出复习节奏建议。
- **FMZH-USR-0063** 管理员可发布统一考试安排，并可维护课程、科目和基础考试信息。
- **FMZH-USR-0064** 管理员可管理异常账号和违规内容。
- **FMZH-USR-0065** 所有用户数据应加密存储，普通学生只能访问和管理自己的个人数据。
- **FMZH-USR-0066** 系统应记录关键操作日志，便于问题排查。
- **FMZH-USR-0067** 系统应在删除账号、删除资料等重要操作前进行二次确认。
...
| 文档 | 说明 |
|---|---|
| 本文件第 2 节 | 9 个模块 57 个用例的状态表（含负责人）+ 67 条用户需求全文（FMZH-USR-0001~0067） |
